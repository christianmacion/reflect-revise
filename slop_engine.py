...
VENDORED (verbatim) from the sibling slop-scanner project
(05_live_demo/slop-scanner/slop_engine.py). Stdlib-only, deterministic,
fully offline. `score_text(text) -> dict` with a `slop_index` (lower is better).

In reflect-revise this scorer is the JUDGE: the reflection loop's task is
"rewrite this draft to lower its SLOP INDEX," and the slop_index gates the loop
(threshold / no-progress / max-iterations).
